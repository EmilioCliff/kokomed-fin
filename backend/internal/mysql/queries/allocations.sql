...
SELECT * FROM payment_allocations WHERE non_posted_id = sqlc.arg("non_posted_id") AND deleted_at IS NOT NULL;

-- name: ListPaymentAllocationsByLoanId :many
SELECT * FROM payment_allocations WHERE loan_id = sqlc.arg("loan_id") AND deleted_at IS NOT NULL;
...
UPDATE payment_allocations SET deleted_at = NOW() WHERE non_posted_id = sqlc.arg("non_posted_id") AND deleted_at IS NOT NULL;
