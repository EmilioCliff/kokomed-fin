...
func newBranchReport(data []services.BranchReportData, format string, filters services.ReportFilters) *branchReport {
...
func (br *branchReport) generateExcel(sheetName string) (error) {
	...
	// br.file.SetColStyle(br.currentSheet, "H", br.createPercentageStyle())
	...
	return br.closeExcel()
}

func (br *branchReport) generatePDF() (error) {
	...
	center := br.getCenterX()
	br.drawBox(marginX, br.pdf.GetY(), center/2, lineHt*3, secondaryColor)
	...
	return br.pdf.OutputFileAndClose("branches_report.pdf")
}