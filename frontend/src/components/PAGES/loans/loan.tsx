...
export const loanColumns: ColumnDef<Loan>[] = [
  ...
  },
  {
    ...
    filterFn: (row, columnId, filterValue) => {
      ...
      const loanOfficerName = row.original.loanOfficer.fullName.toLowerCase();
      return (
  ...
  {
    accessorKey: 'feePaid',
    header: ({ column }) => <DataTableColumnHeader column={column} title="Fee Paid" />,
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="">{Number(row.getValue('paidAmount')).toLocaleString()}</div>
    ),
    enableSorting: true,
  },
];
