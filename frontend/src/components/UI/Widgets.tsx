  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        {title === 'Customers' && <Users className="h-4 w-4 text-muted-foreground" />}
        {title === 'Loans' && <Wallet className="h-4 w-4 text-muted-foreground" />}
        {title === 'Transactions' && <Flag className="h-4 w-4 text-muted-foreground" />}
        {title === 'Payments' && <DollarSign className="h-4 w-4 text-muted-foreground" />}
      </CardHeader>
      <CardContent>
